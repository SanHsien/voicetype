...
- **依賴更新一律人工審查**：`.github/dependabot.yml` 每週檢查 pip 與 GitHub Actions，`.github/workflows/dependency-freshness.yml` 每月（以及依賴宣告推上 `main` 時）更新維護 issue，CodeQL 每週掃描。VoxProse 的 PyQt6／Whisper／ONNX Runtime／CUDA 與 Release actions 會影響 Windows 真機或可攜包，**不得自動核准或自動合併 Dependabot PR**。先讀 changelog、確認 Python 3.10–3.14 Windows wheel，跑完整 CI；涉及錄音、STT、CUDA、UI 或打包鏈時，再做對應 Windows 實機／Release 驗證。
